...
async function fetchArbres() {
    ...
    const limit = 100; // Nombre maximum d'arbres par appel
    ...
    try {
        ...
        if (response.status === 429) {
            // Gestion de la limite de requêtes
        ...
        if (data.records && Array.isArray(data.records)) {
            ...
            cacheArbres.data = data.records;
            cacheArbres.timestamp = currentTime;
...
function afficherArbres(arbres) {
    arbres.forEach((arb) => {
        const lat = arb.geo_point_2d[0]; // Latitude
        const lng = arb.geo_point_2d[1]; // Longitude
        const nom = arb.libellefrancais || "Arbre"; // Nom de l'arbre
        const adresse = arb.adresse || "Adresse inconnue"; // Adresse de l'arbre

        // Vérifiez si le marqueur existe déjà dans le tableau
        const existingMarker = arbreMarkers.find(marker => marker.getLatLng().lat === lat && marker.getLatLng().lng === lng);
        
        if (lat !== undefined && lng !== undefined) {
            if (existingMarker) {
                // Si le marqueur existe, mettez à jour son contenu
                existingMarker.setPopupContent(`<b>${nom}</b><br>${adresse}`);
            } else {
                // Créez un nouveau marqueur si non existant
